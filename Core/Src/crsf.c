...
static void crsf_unpack_channels(const uint8_t *p, CRSF_Data_t *d)
{
     ...
     */
    d->channels[0]  = ((uint32_t)p[0]         | (uint32_t)p[1]  << 8)  & 0x07FFU; /* bits  0-10 */
    d->channels[1]  = ((uint32_t)p[1]  >> 3   | (uint32_t)p[2]  << 5)  & 0x07FFU; /* bits 11-21 */
    d->channels[2]  = ((uint32_t)p[2]  >> 6   | (uint32_t)p[3]  << 2
                     | (uint32_t)p[4]  << 10) & 0x07FFU;                           /* bits 22-32 */
    d->channels[3]  = ((uint32_t)p[4]  >> 1   | (uint32_t)p[5]  << 7)  & 0x07FFU; /* bits 33-43 */
    d->channels[4]  = ((uint32_t)p[5]  >> 4   | (uint32_t)p[6]  << 4)  & 0x07FFU; /* bits 44-54 */
    d->channels[5]  = ((uint32_t)p[6]  >> 7   | (uint32_t)p[7]  << 1
                     | (uint32_t)p[8]  << 9)  & 0x07FFU;                           /* bits 55-65 */
    d->channels[6]  = ((uint32_t)p[8]  >> 2   | (uint32_t)p[9]  << 6)  & 0x07FFU; /* bits 66-76 */
    d->channels[7]  = ((uint32_t)p[9]  >> 5   | (uint32_t)p[10] << 3)  & 0x07FFU; /* bits 77-87 */
    /* --- Bug fix: CH8 bắt đầu tại bit 88 = byte p[11] bit0 (không phải p[11] skip) --- */
    d->channels[8]  = ((uint32_t)p[11]        | (uint32_t)p[12] << 8)  & 0x07FFU; /* bits 88-98  */
    d->channels[9]  = ((uint32_t)p[12] >> 3   | (uint32_t)p[13] << 5)  & 0x07FFU; /* bits 99-109 */
    d->channels[10] = ((uint32_t)p[13] >> 6   | (uint32_t)p[14] << 2
                     | (uint32_t)p[15] << 10) & 0x07FFU;                           /* bits 110-120 */
    d->channels[11] = ((uint32_t)p[15] >> 1   | (uint32_t)p[16] << 7)  & 0x07FFU; /* bits 121-131 */
    d->channels[12] = ((uint32_t)p[16] >> 4   | (uint32_t)p[17] << 4)  & 0x07FFU; /* bits 132-142 */
    d->channels[13] = ((uint32_t)p[17] >> 7   | (uint32_t)p[18] << 1
                     | (uint32_t)p[19] << 9)  & 0x07FFU;                           /* bits 143-153 */
    d->channels[14] = ((uint32_t)p[19] >> 2   | (uint32_t)p[20] << 6)  & 0x07FFU; /* bits 154-164 */
    d->channels[15] = ((uint32_t)p[20] >> 5   | (uint32_t)p[21] << 3)  & 0x07FFU; /* bits 165-175 */
}
 ...
 * ========================================================================== */
static void crsf_handle_frame(const uint8_t *p, uint8_t type,
                               uint8_t pay_len, CRSF_Data_t *data)
{
    ...
    case CRSF_FRAMETYPE_RC_CHANNELS:
        if (pay_len == CRSF_PAYLOAD_SIZE_RC_CHANNELS) {
            ...
            /* [DEBUG] In ra giá trị của 3 kênh quan trọng mỗi 500ms để tránh lag console */
            static uint32_t last_print = 0;
            if (HAL_GetTick() - last_print > 500) {
                printf("[CRSF] CH1 (Servo1): %lu, CH2 (Servo2): %lu, CH6 (Light): %lu\n", 
                       (unsigned long)data->channels[0], 
                       (unsigned long)data->channels[1], 
    ...
    case CRSF_FRAMETYPE_BATTERY_SENSOR:
        if (pay_len >= CRSF_PAYLOAD_SIZE_BATTERY) {
            ...
            uint32_t raw_c = ((uint32_t)p[4] << 16)
                           | ((uint32_t)p[5] << 8)
                           |  (uint32_t)p[6];
...
{
    memset(data, 0, sizeof(CRSF_Data_t));
}
                         ...
                         CRSF_Data_t *data)
{
    ...
    for (uint16_t i = 0; i < length; i++) {
         ...
         * Byte sync này có thể là false positive (byte dữ liệu có giá trị trùng địa chỉ). */
        if (crc_idx >= length) {
            continue;
        }
        ...
        uint8_t crc_calc     = crsf_crc8(&buffer[i + 2U],
...
void CRSF_Input_Update(void)
{
    ...
    if (write_pos != s_read_pos)
    {
        ...
        else
        {
            ...
            if (bytes_to_end > 0U) {
                CRSF_ParseFrame(&s_dma_buf[s_read_pos], bytes_to_end,
                                &s_crsf_data);
            }
